...
const RestaurantCard = ({ restaurant, onClick, formatPrice }) => (
  <Box sx={{ width: { xs: '100%', sm: 'calc(50% - 8px)', md: 'calc(33.333% - 11px)' }, maxWidth: { xs: '400px', sm: 'calc(50% - 8px)', md: 'calc(33.333% - 11px)' }, minWidth: { xs: '300px', sm: '280px', md: '280px' }, flex: '0 0 auto' }}>
    <Card
      sx={{ cursor: 'pointer', height: '380px', width: '100%', minWidth: 0, maxWidth: '100%', borderRadius: '3px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', transition: 'all 0.3s ease', overflow: 'hidden', display: 'flex', flexDirection: 'column', backgroundColor: 'white', flex: '1 1 auto', '&:hover': { boxShadow: '0 8px 24px rgba(0,0,0,0.15)', transform: 'translateY(-4px)' } }}
      ...
      <CardContent sx={{ p: 2.5, flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '200px', width: '100%', maxWidth: '100%', minWidth: 0, overflow: 'hidden', boxSizing: 'border-box' }}>
        <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 1 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 0.5 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, flexShrink: 0, backgroundColor: '#f8f9fa', px: 1, py: 0.5, borderRadius: '3px', border: '1px solid #e0e0e0' }}>
